...
describe('useSessionTimeline', () => {
  it('メッセージのみのとき全て preamble、sections は空', () => {
    const { result } = renderHook(() =>
      useSessionTimeline(
        [msg('m1', '2026-04-18T10:00:00+09:00'), msg('m2', '2026-04-18T10:05:00+09:00')],
        [],
      ),
    )
    expect(result.current.preamble.map((p) => p.data.id)).toEqual(['m1', 'm2'])
  ...
  it('draft があり、それより前のメッセージは preamble、後のメッセージはセクションに入る', () => {
    ...
    expect(result.current.preamble.map((p) => p.data.id)).toEqual(['m-before'])
    expect(result.current.sections).toHaveLength(1)
    expect(result.current.sections[0].draft.data.exerciseId).toBe('d1')
    expect(result.current.sections[0].messages.map((m) => m.data.id)).toEqual([
      'm-after',
    ])
  })
  ...
  it('複数 draft が時系列順にセクション化され、各セクションは次の draft までのメッセージを保持する', () => {
    ...
    expect(result.current.preamble).toEqual([])
    expect(result.current.sections).toHaveLength(2)
    expect(result.current.sections[0].messages.map((m) => m.data.id)).toEqual(['m1'])
    expect(result.current.sections[1].messages.map((m) => m.data.id)).toEqual([
      'm2',
      'm3',
    ])
  })
  ...
  it('draft の index は draftExercises 配列内の位置を保持する', () => {
    ...
    expect(result.current.sections[0].draft.index).toBe(0)
    expect(result.current.sections[1].draft.index).toBe(1)
  })
})
