/**
 * shadcn/ui の汎用 Calendar ラッパー（現時点では未使用）。
 *
 * 履歴画面の `MonthCalendar` は FRAME 3 の独自スタイル（36px丸セル、赤ドット
    ...
    <DayPicker
      ...
      components={{
        ...
        Chevron: ({ className, orientation, ...props }) => {
          if (orientation === "left") {
            return (
              <CaretLeft weight="bold" className={cn("size-4", className)} {...props} />
            )
          }

          if (orientation === "right") {
            return (
              <CaretRight weight="bold" className={cn("size-4", className)} {...props} />
            )
          }

          return (
            <CaretDown weight="bold" className={cn("size-4", className)} {...props} />
          )
        },
        DayButton: ({ ...props }) => (
