...
ユーザーが日々のトレーニング内容を記録・管理する中核機能。ワークアウトは日付に紐づき、複数の種目とセットで構成される。セッション開始（FRAME1）→ タイムラインで種目カードと AI 対話を並走（FRAME2）→ 終了・保存（FRAME1に戻る）のフローで運用する。
...
1回のセッション内で複数の種目を連続して追加・記録できる。種目の追加は **タイムライン下端の単一入力欄** から行う（[ai-chat/index.md](../ai-chat/index.md) FR_035）。

- 入力欄に種目名（例: 「ベンチ」）を入力すると候補チップが popover で提示され、タップで種目カードを追加
- 自然言語コマンド（例: 「ベンチ60kg×10×3で記録」）を送ると、AI が draft カードとして提案する（REQ_008）

旧「種目を追加...」専用検索フィールド（ExerciseSearchField）は段階的に撤去される。
...
| 状態 | 説明 | UI |
|:-----|:-----|:---|
| **折りたたみ（collapsed）** | セット一覧非表示。セット数サマリー表示 | opacity-70、caret-down |
| **待機（idle）** | 完了済みセット（あれば）+ 「+」ボタン。入力行なし | 「+」ボタンで記録中へ遷移 |
| **記録中（recording）** | 完了済みセット + 入力中セット行。同時に1種目のみ | 入力行に左端黒バー |
...
- `recording` カードは `position: sticky` で上部固定（同時に1種目のみ recording に入れるルールは FR_030）
