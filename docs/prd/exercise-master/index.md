...
- 検索: リアルタイム絞り込み
- 種目行: 部位カラードット + 名前 + 部位ラベル + 編集ボタン（鉛筆アイコン `ph-pencil-simple`）
- 追加: `+` アイコン + 「種目を追加」ラベル
- 部位（category）: 追加・編集フォームで単一選択チップ（胸/背中/肩/腕/脚/体幹/有酸素/未分類）から割り当てる。既定は「未分類」。旧データ（category なし）は「未分類」に移行される。部位はカレンダーの色分け（history FR_014）に用いる。
