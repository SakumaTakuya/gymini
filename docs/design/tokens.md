...
種目の対象部位を色分けする（履歴カレンダーの日付ドット・凡例、種目マスターの行/選択チップ）。
モノクロ＋アクセント赤の基調に対する例外として、部位識別のための多色パレットを持つ。

- **唯一の定義元**: `src/lib/exerciseCategory.ts` の `CATEGORY_META`（`label` と `color` の hex）。
  Tailwind の動的クラスが使えないため CSS トークンには複製せず、ドット/チップは inline style で色を参照する。
- 部位: 胸 `#DE3A2B`（= gym-accent）/ 背中 `#2563EB` / 肩 `#F59E0B` / 腕 `#9333EA` / 脚 `#16A34A` / 体幹 `#0891B2` / 有酸素 `#EC4899` / 未分類 `#A1A1AA`（zinc-400 相当）。
