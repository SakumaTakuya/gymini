...
test.describe('設定画面の種目管理 (Task 4.1)', () => {
  ...
  test('手動追加→追加された種目が表示される', async ({ page }) => {
    await page.getByRole('button', { name: 'Settings' }).click()
  ...
  test('重複名で追加するとエラーが表示される', async ({ page }) => {
    await page.getByRole('button', { name: 'Settings' }).click()
...
test.describe('自動登録フロー (Task 4.2)', () => {
  ...
  test('TrainingPage: 未登録種目を検索→新規追加→セッションに追加', async ({ page }) => {
    // Start session
  ...
  test('AddExerciseModal: 未登録種目を検索→新規追加→セッションに追加', async ({ page }) => {
    // Start session
    await page.getByRole('button', { name: 'トレーニングを開始' }).click()
  ...
  test('自動登録した種目が設定画面にも反映される', async ({ page }) => {
