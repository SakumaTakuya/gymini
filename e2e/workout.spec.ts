...
test.describe('トレーニング記録フロー', () => {
  ...
  test('完了済みセットの編集', async ({ page }) => {
    ...
    // 鉛筆ボタンで編集 (P1: sr-only ボタンなので dispatchEvent 経由)
    await page.getByRole('button', { name: '編集' }).dispatchEvent('click')
  ...
  test('完了済みセットの削除', async ({ page }) => {
    ...
    // ゴミ箱ボタンで1つ削除 (P1: sr-only ボタンなので dispatchEvent 経由)
...
test.describe('セット編集バグ修正', () => {
  ...
  test('編集完了後にセットが元の位置に戻る（末尾追加されない）', async ({ page }) => {
    ...
    // 1番目のセット（60kg）を編集
    await page.getByRole('button', { name: '編集' }).first().dispatchEvent('click')
