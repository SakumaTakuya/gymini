...
- **最小限のルーティング**: 外部ルーターライブラリを使わず、状態ベースの軽量ルーティング
...
| FR-005 | 設定ページ（FRAME5）を歯車アイコンから遷移可能にし、Xボタンで遷移元に戻れるようにする | 必須 | IR_002 | Test |
...
- ルーティングは状態ベースで実現する（DC_005, A-001例外: 4ルートのみで外部ルーターは過剰。詳細な根拠は [navigation_design.md](navigation_design.md) を参照）
